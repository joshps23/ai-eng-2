...
def highlight_block(code: str, lang: str) -> str:
...
class GuidePreprocessor(Preprocessor):
    """Three fence-aware source fixes:
    ...
    def run(self, root: etree.Element) -> None:
        ...
        for el in root.iter():
            ...
            if tag in ("h1", "h2", "h3", "h4", "h5", "h6"):
                ...
                href = el.get("href")
                if href:
                    el.set("href", rewrite_href(href, self.ctx.source_dir))
...
document.querySelectorAll('div.highlight').forEach(function (block) {
  ...
  btn.addEventListener('click', function () {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      ...
      }).catch(function () {});  /* graceful no-op on file:// denial */
    }
  });
...
def build_toc(toc: list[tuple[int, str, str]]) -> str:
    ...
    for level, text, slug in toc:
        items.append(f'<li class="toc-h{level}"><a href="#{slug}">{esc(text)}</a></li>')
    return (
...
<p class="generated-note">Generated from <code>{esc(src_rel)}</code> — the markdown is the source of truth.</p>
</footer>
</main>
</div>
<script>
...
.sidebar h2 {
  ...
  color: var(--muted); margin: 1.25em 0 0.25em;
}
...
.page-toc li.toc-h3 { padding-left: 1.5em; }
